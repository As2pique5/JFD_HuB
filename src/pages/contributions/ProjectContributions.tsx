...
export default function ProjectContributions() {
  ...
  const updateProjectStats = useContributionStore(state => state.updateProjectStats);
  ...
  // Fetch and update project contribution stats
  useEffect(() => {
    const fetchProjectStats = async () => {
      try {
        ...
        updateProjectStats({
          projectContributions: totalCollected,
          projectTargetAmount: totalTarget,
          projectCompletionRate: totalProjects > 0 ? (completedProjects / totalProjects) * 100 : 0,
          projectPaymentRate: totalTarget > 0 ? (totalCollected / totalTarget) * 100 : 0,
        });
      } catch (error) {
    ...
    fetchProjectStats();
  }, [yearFilter, updateProjectStats]);

  // Calculate total project contributions
  const totalProjectContributions = allMembers.reduce((sum, member) => sum + member.totalContribution, 0);
  ...
  return (
    <div className="space-y-6">
      ...
      <ProjectContributionList searchTerm={searchTerm} yearFilter={parseInt(yearFilter)} />
              ...
              <table className="w-full">
                    ...
                    <tr key={member.id} className="hover:bg-muted/50">
                      ...
                      <td className="px-4 py-2 text-sm text-right text-muted-foreground">
                        {totalProjectContributions > 0 
                          ? ((member.totalContribution / totalProjectContributions) * 100).toFixed(1)
                          : '0.0'}%
                      </td>
                    </tr>
                  ))}
                ...
                <tfoot className="bg-muted/30">
                  <tr>
                    ...
                    <td className="px-4 py-2 text-sm text-right font-medium text-foreground">
                      {formatCurrency(totalProjectContributions)}
                    </td>
