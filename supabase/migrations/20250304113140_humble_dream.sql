...
-- Créer une politique qui vérifie l'unicité de l'email lors de l'insertion
CREATE POLICY "Anyone can insert profiles"
  ON profiles FOR INSERT
  WITH CHECK (
    NOT EXISTS (
      SELECT 1 FROM profiles WHERE profiles.email = current_setting('request.jwt.claims', true)::json->>'email'
    )
  );